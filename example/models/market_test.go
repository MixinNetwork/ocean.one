...
func TestMarketCRUD(t *testing.T) {
	...
	assert.NotNil(err)
	assert.Nil(fm)
	fm, err = user.LikeMarket(ctx, "c6d0c728-2624-429b-8e0d-d9d19b6592fa", "4d8c508b-91c5-375b-92b0-ee702ed2dac5")
	assert.Nil(err)
	assert.NotNil(fm)
	...
	assert.Nil(err)
	assert.Len(set, 1)
	err = user.DislikeMarket(ctx, "c6d0c728-2624-429b-8e0d-d9d19b6592fa", "4d8c508b-91c5-375b-92b0-ee702ed2dac5")
	assert.Nil(err)
	set, err = readFavoriteMarkets(ctx, user)
	assert.Nil(err)
	assert.Len(set, 0)
	...
	err = CreateOrUpdateMarket(ctx, "c6d0c728-2624-429b-8e0d-d9d19b6592fa", "4d8c508b-91c5-375b-92b0-ee702ed2dac5", 0, 0, 0, 0, 0)
	assert.Nil(err)
